...
public class ReplicatedIncomingServerEventProcessor extends AbstractReplicatedEventProcessor {
  ...
  private void publishIncomingToEventStream(Event event) {
    if(!replicatedEventsCoordinator.getReplicatedConfiguration().isReplicatedEventsEnabled()){
      return;
    }
    ...
    if (replicatedEventsCoordinator.getReplicatedConfiguration().isReplicatedEventsReplicateOriginalEvents()) {
